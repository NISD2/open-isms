...
export async function generateMetadata({
  ...
  const title = t("amIMachineryManufacturer.meta.title");
  const description = t("amIMachineryManufacturer.meta.description");
  return {
    title,
    description,
    alternates: pageAlternates(
      "wiki/anwendungsbereich/bin-ich-maschinenbauer-nis2",
      locale,
    ),
    ...pageOg({
      slug: "wiki/anwendungsbereich/bin-ich-maschinenbauer-nis2",
      locale,
      title,
      description,
      type: "article",
...
const elementKeys = ["nace", "size", "otIt"] as const;
const principleKeys = ["annexIIWichtig", "otInScope"] as const;
const nationalKeys = ["bsi", "vdma", "enisa"] as const;
const pitfallKeys = ["onlyBigCompanies", "justAssembly", "nonEuBuyers"] as const;

export default async function AmIMachineryManufacturerPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
    ...
    <GlossedProse locale={locale}>
      <div className="space-y-10">
        <WikiPageJsonLd
          ...
          slug="bin-ich-maschinenbauer-nis2"
          locale={locale}
          authorSlug="simon-orzel"
          proficiencyLevel="Beginner"
          audienceType="Geschäftsführung, CISO und IT-Verantwortliche im Maschinen- und Anlagenbau"
        ...
        <header>
          ...
          <h1 className="text-3xl font-bold tracking-tight">
            {t("amIMachineryManufacturer.title")}
          </h1>
          <p className="mt-2 text-lg text-muted-foreground">
            {t("amIMachineryManufacturer.subtitle")}
          </p>
        </header>
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold tracking-tight">
            {t("amIMachineryManufacturer.overview.heading")}
          </h2>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("amIMachineryManufacturer.overview.p1")}
          </p>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("amIMachineryManufacturer.overview.p2")}
          </p>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("amIMachineryManufacturer.overview.p3")}
          </p>
        </section>
              ...
              {anchorKeys.map((key) => (
                <div key={key} className="rounded-lg border p-4">
                  <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    {t(`amIMachineryManufacturer.legalAnchor.${key}.label`)}
                  </p>
                  <blockquote className="mt-2 border-l-2 border-primary/40 pl-3 text-sm italic leading-relaxed">
                    {t(`amIMachineryManufacturer.legalAnchor.${key}.quote`)}
                  </blockquote>
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                    {t(`amIMachineryManufacturer.legalAnchor.${key}.context`)}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {elementKeys.map((key) => (
                <div key={key} className="rounded-lg border p-4">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                    {t(`amIMachineryManufacturer.elements.items.${key}.body`)}
                  </p>
                </div>
              ))}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm leading-relaxed text-muted-foreground">
              {t("amIMachineryManufacturer.practitioner.p1")}
            </p>
            <p className="text-sm leading-relaxed text-muted-foreground">
              {t("amIMachineryManufacturer.practitioner.p2")}
            </p>
          </CardContent>
        </Card>
