...
export async function generateMetadata({
  ...
  const title = t("howToConductRiskAssessment.meta.title");
  const description = t("howToConductRiskAssessment.meta.description");
  return {
    title,
    description,
    alternates: pageAlternates(
      "wiki/umsetzung/wie-nis2-risikoanalyse-durchfuehren",
      locale,
    ),
    ...pageOg({
      slug: "wiki/umsetzung/wie-nis2-risikoanalyse-durchfuehren",
      locale,
      title,
      description,
      type: "article",
...
const elementKeys = ["scope", "threats", "treatment"] as const;
const principleKeys = ["proportionality", "annualReview"] as const;
const nationalKeys = ["bsi", "enisa", "iso"] as const;
const pitfallKeys = ["doneForever", "everyCve", "blanketAccept"] as const;

export default async function HowToConductRiskAssessmentPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <GlossedProse locale={locale}>
      <div className="space-y-10">
        <WikiPageJsonLd
          category="umsetzung"
          slug="wie-nis2-risikoanalyse-durchfuehren"
          locale={locale}
        ...
        <header>
          ...
          <h1 className="text-3xl font-bold tracking-tight">
            {t("howToConductRiskAssessment.title")}
          </h1>
          <p className="mt-2 text-lg text-muted-foreground">
            {t("howToConductRiskAssessment.subtitle")}
          </p>
        </header>
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold tracking-tight">
            {t("howToConductRiskAssessment.overview.heading")}
          </h2>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("howToConductRiskAssessment.overview.p1")}
          </p>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("howToConductRiskAssessment.overview.p2")}
          </p>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("howToConductRiskAssessment.overview.p3")}
          </p>
        </section>
              ...
              {anchorKeys.map((key) => (
                <div key={key} className="rounded-lg border p-4">
                  <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    {t(`howToConductRiskAssessment.legalAnchor.${key}.label`)}
                  </p>
                  <blockquote className="mt-2 border-l-2 border-primary/40 pl-3 text-sm italic leading-relaxed">
                    {t(`howToConductRiskAssessment.legalAnchor.${key}.quote`)}
                  </blockquote>
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                    {t(`howToConductRiskAssessment.legalAnchor.${key}.context`)}
                  </p>
                </div>
              ))}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm leading-relaxed text-muted-foreground">
              {t("howToConductRiskAssessment.practitioner.p1")}
            </p>
            <p className="text-sm leading-relaxed text-muted-foreground">
              {t("howToConductRiskAssessment.practitioner.p2")}
            </p>
          </CardContent>
        </Card>
