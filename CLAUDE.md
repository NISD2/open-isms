...
- **Refunds are manual and live in the Subscriptions tab** (`lib/billing/subscriptions.ts`). A credit note moves no money and our key cannot send a transfer, so a refund owed stays on the tab until a platform admin marks it done (`refund_done_at` and `_by`, audited). Revoke sets a full account to `unpaidAccessLevel` by hand, audited. Door two ("New customer") lives on that tab too.
