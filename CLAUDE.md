...
- **One order service, `placeOrder` in `lib/billing/place-order.ts`.** The order page (tRPC `billing.place`) calls it, the admin close must call it too when it is built, and nothing else creates an invoice. The order page only orders at a price the customer was shown (`quotedGrossCents`). The price comes from the server, never from a browser.
- **One function decides the price: `holderNetCents` (`lib/billing/holder-price.ts`).** It prices the account holder, not the account level: a grandfathered person (`isGrandfatheredPerson` in `lib/billing/access.ts`: stamped `grandfatheredAt`, or before the launch anyone who has got in) pays 2.400 net on every order, renewal and re-order, even while their account is full; everyone else pays 4.800. `billing.status`, `billing.quote`, `placeOrder` and `closeNetCents` all call it. Never price from `accessLevel`.
- **One cancel service, `cancelSubscription` in `lib/billing/cancel.ts`**, behind `billing.cancel` (`payerProcedure`, the account holder only). Inside the thirty days (`cancelWindow` in `lib/billing/cancel-terms.ts`: the order day is day 0, day 30 is the last, counted in Berlin) it issues a full credit note in Qonto, which cancels the invoice paid or not, writes the `credit_note` row and sets the account to `unpaidAccessLevel` (grandfathered for a grandfathered holder, else free). `refund_owed` is Qonto's live status read just before the credit note: `paid` means owed; any status other than paid or unpaid refuses and alerts. After day 30 it only sets `renewal_canceled_at`; access runs to `period_end`. The credit note follows the order's safety pattern exactly: number from `document_number_counter` series `credit_note` (prefix `GS`, committed before the call, sandbox never touches it), the account lock, an `order_check` mark with the GS number, `alertOperators` on an unclear answer. The confirmation is `billing.canceled`, with the credit note PDF from `lib/billing/deliver-credit-note.ts`, archived to `billing/<account>/<GS number>.pdf`.
- **Refunds are manual and live in the Subscriptions tab** (`lib/billing/subscriptions.ts`). A credit note moves no money and our key cannot send a transfer, so a refund owed stays on the tab until a platform admin marks it done (`refund_done_at` and `_by`, audited). **"Unpaid" in Qonto is not proof that no money came:** customers pay by transfer, Qonto flips to paid only once it matches one, and a credited invoice never flips afterwards. So every credit note on an unpaid invoice alerts the operators and sits under "Watch for a late payment" for ninety Berlin calendar days (derived from `refund_owed = false` and `created_at`, no flag); "Payment arrived, refund owed" sets `refund_owed` when a transfer turns up. Customer copy never tells an unpaid canceler there is nothing more to do. Revoke sets a full account to `unpaidAccessLevel` by hand, audited. Door two ("New customer") lives on that tab too.
- **Who may order is `lib/billing/ordering.ts` plus the account holder.** Off while Qonto or the invoice prefix is unusable, platform admins only against the sandbox, everyone live; and only `billing_account.ownerUserId` (`payerProcedure`). Never a company role: any member may add an organization and is its admin. `billingFor` (`lib/billing/ordering-access.ts`) decides whether billing exists for a visitor: `/bestellen`, `/billing` and the sidebar entry. Platform admins get it whenever Qonto is configured; everyone else needs live keys AND pricing launched (`feature_flag` row `billing`). Setting the keys launches nothing.
- **The pricing launch is one-way, from the platform admin Pricing tab** (`lib/billing/launch.ts`). In one transaction it stamps `user.grandfatheredAt` on everyone who has ever got in, moves their free accounts to grandfathered, freezes them into a newsletter group for the announcement, and sets the switch. Nothing in the app turns it off. **Grandfathering belongs to the person:** an account opened later by a stamped person starts grandfathered too (`newAccountAccessLevel`).
- **The access gate is on by default** (`server/trpc/init.ts`): `companyProcedure`, `adminProcedure` and `reviewerProcedure` refuse an effective free account. `accountProcedure` / `accountAdminProcedure` are the only exemptions, used by billing, notifications, `assessment.updateCompany` and the supplier portal. A new router is gated unless it deliberately uses an account tier. The portal layout sends a free account to `/bestellen` except for `FREE_PORTAL_PATHS` (`lib/billing/access.ts`).
- **Nothing on our side may fail after Qonto has issued an invoice or a credit note.** Everything that can refuse (the account lock, an unexpired uncredited invoice, a price that moved since the quote, which Qonto client this account may use) is decided before `createInvoice`. A Qonto client belongs to one billing account, so a client another account holds is never reused. When Qonto's answer is unclear (timeout, 5xx, or the order breaking part way), an invoice may exist: the customer is told not to retry, `alertOperators` mails the platform admins, and an `order_check` row blocks every further order for that account on the server until a platform admin clears it in the Pricing tab (`lib/billing/order-check.ts`). The row is committed on its own just before the Qonto call and deleted inside the order's transaction once Qonto answered clearly, so it exists whenever an invoice might, and never for a failure before the call. The order therefore locks its account `FOR NO KEY UPDATE`: `FOR UPDATE` would make that insert, whose foreign key points at the locked row, wait on the order itself. Every case a person must act on goes through that alert, never only a log line.
...
- **Door two is `closeDeal`** (`lib/billing/close-deal.ts`, platform admin Subscriptions tab): finds or creates the customer's user (no password, a draft company and its own account through `createDraftCompany`), then calls the same `placeOrder` with `source: "admin"` and an optional agreed net (`netCentsOverride`, admin only; door one always passes null). A customer who has never signed in gets an `account.setup` link: a seven-day, single-use token in `email_otp` (purpose `account_setup`, sha256 of the secret, `lib/auth/setup-link.ts`), opening `/auth/setup` to set a password or continue with Google. Both doors render the same fields (`components/billing/OrderFields.tsx`) and quote through `lib/billing/quote.ts`.
...
- **Every company gets a billing account** in the transaction that creates it (`lib/billing/accounts.ts`). A further company started from the switcher joins the account of the open one, so it inherits its access level; any member of a set-up company may add one, because one payment covers unlimited companies.
