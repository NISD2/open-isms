...
- **One order service, `placeOrder` in `lib/billing/place-order.ts`.** The order page (tRPC `billing.place`) calls it, the admin close must call it too when it is built, and nothing else creates an invoice. The order page only orders at a price the customer was shown (`quotedGrossCents`). The price comes from the account's access level on the server, never from a browser.
