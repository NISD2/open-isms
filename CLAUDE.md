...
- **Refunds are manual and live in the Subscriptions tab** (`lib/billing/subscriptions.ts`). A credit note moves no money and our key cannot send a transfer, so a refund owed stays on the tab until a platform admin marks it done (`refund_done_at` and `_by`, audited). **"Unpaid" in Qonto is not proof that no money came:** customers pay by transfer, Qonto flips to paid only once it matches one, and a credited invoice never flips afterwards. So every credit note on an unpaid invoice alerts the operators and sits under "Watch for a late payment" for thirty days (derived from `refund_owed = false` and `created_at`, no flag); "Payment arrived, refund owed" sets `refund_owed` when a transfer turns up. Customer copy never tells an unpaid canceler there is nothing more to do. Revoke sets a full account to `unpaidAccessLevel` by hand, audited. Door two ("New customer") lives on that tab too.
