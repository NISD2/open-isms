...
- `CRON_SECRET` — Bearer token for `/api/cron/deadlines`, `/api/cron/course-reminders` and `/api/cron/lifecycle`
...
All three endpoints reject requests without a matching `CRON_SECRET` bearer
