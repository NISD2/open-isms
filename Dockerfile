...
# nis2-supply-chain-questionnaire-schema ships its `dist/` to GitHub and
# consumers (PDF / DOCX routes + public viewer) import from there per the
# package's `main: ./dist/index.js`. Copying the whole package — not just
# package.json — so dist/ exists at bun-install time and Turbopack can
# resolve the module during `next build`. Slight loss of deps-layer cache
# on schema edits is acceptable; rebuilds are rare.
COPY packages/nis2-supply-chain-questionnaire-schema ./packages/nis2-supply-chain-questionnaire-schema
