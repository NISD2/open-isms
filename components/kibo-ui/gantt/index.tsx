...
export type GanttMarkerProps = {
  ...
  date: Date;
  label: string;
  /** Pixel offset for the sticky label so neighbouring markers can stagger. */
...
const createInitialTimelineData = (
  today: Date,
  options?: { startYear?: number; yearsCount?: number }
) => {
  const data: TimelineData = [];

  // Default behaviour: 3-year window centered on today (prev/curr/next).
  // Callers can pass an explicit startYear + yearsCount to trim padding,
  // e.g. nisd2.eu business-plan dashboard which knows its data lives in
  // 2026-2028 and doesn't want a Q1 2025 column eating screen space.
  const startYear = options?.startYear ?? today.getFullYear() - 1;
  const yearsCount = options?.yearsCount ?? 3;
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center gap-2.5 p-2.5 text-sm hover:bg-secondary",
        className
      )}
      key={feature.id}
      onClick={handleClick}
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <p
      className="w-full truncate p-2.5 text-left font-semibold text-muted-foreground text-sm uppercase tracking-wide"
...
export type GanttFeatureItemCardProps = Pick<GanttFeature, "id"> & {
  children?: ReactNode;
  /** Fill color for the bar background (typically a soft tint of the phase color). */
  fillColor?: string;
  /** Border color for the bar (typically the phase's status color). */
  borderColor?: string;
};
      ...
      >
        ...
        >
          <GanttFeatureItemCard
            id={feature.id}
            fillColor={`${feature.status.color}1f`}
...
> = memo(({ label, date, id, onRemove, className, verticalOffset = 0 }) => {
  const gantt = useContext(GanttContext);
...
export type GanttProviderProps = {
  ...
  children: ReactNode;
  className?: string;
  /** First year on the timeline. Defaults to last year (today.getFullYear() - 1). */
  startYear?: number;
  /** How many years to render. Defaults to 3. */
  yearsCount?: number;
};
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const [timelineData, setTimelineData] = useState<TimelineData>(
    createInitialTimelineData(new Date(), { startYear, yearsCount })
  );
  const [, setScrollX] = useGanttScrollX();
  const [sidebarWidth, setSidebarWidth] = useState(0);

  // Bumped from the Kibo default (60 / 36) to 76 / 44 for readability in
  // the screenshot-for-PDF use case.
