...
export function CustomerIncidentsSection({
  ...
  const assetsQuery =
    trpc.supplierPortal.managedAsset.listByRelationship.useQuery({
      relationshipId,
    });
    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Notify {recipientLabel} about a security event affecting the assets
          you manage for them. They use this as evidence for their NIS2 §30
          supplier monitoring obligation.
        </p>
      </header>
      ...
      >
        <h3 className="font-medium text-sm">Publish a new incident</h3>
        ...
        <div>
          ...
          {assetsQuery.isLoading ? (
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">
              No assets declared for this customer yet. The notification will
              go out without asset attribution.
            </p>
          )}
        </div>
      ...
      {/* History */}
      <section>
        <h3 className="font-medium mb-3">Published incidents</h3>
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <span className="capitalize">
                    Broadcast: {event.broadcastStatus ?? "n/a"} (
                    {event.broadcastCount ?? 0} sent)
                  </span>
                </div>
              </li>
            ))}
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <Badge
      variant={variants[severity] ?? "secondary"}
