...
| Mail that used to arrive has stopped | Open the **email** tab in `/platform-admin`. A **Failed sends** card lists what failed, for whom, and the reason the transport gave, for the last 30 days. The same lines are in the container log: `docker compose logs app \| grep "\[mail\] send failed"`. |
