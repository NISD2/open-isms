...
function buildSystemPrompt(
  ...
  const parts = [
    ...
    "- `signedOffBy` (authenticated user ID)",
    ...
    "Under eIDAS this is a simple electronic signature (SES) carrying a tamper-evident audit trail: authenticated identity, server timestamp, frozen snapshot and a chained SHA-256 checksum. It is NOT an advanced electronic signature (AES), because no signing key sits under the signatory's sole control. Do not credit the platform with AES or with qualified signatures.",
